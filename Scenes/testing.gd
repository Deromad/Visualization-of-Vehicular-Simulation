...
func building_2d5(json) -> bool:
	...
	if not json.has("filled") or not test_bool(json["filled"]):
		append_error("Building_2D5 with id: " + str(json["id"]) + " has invalid filled property")
		return false

	if not json.has("lineWidth") or not test_line_width(json["lineWidth"]):
		append_error("Building_2D5 with id: " + str(json["id"]) + " has invalid lineWidth")
		return false
...
func junction(json) -> bool:
	...
	if not json.has("pos") or not D3_position(json["pos"]):
		append_error("Junction with id: " + str(json["id"]) + " has no or invalid position")
		return false
...
func test_lane(json) -> bool:
	...
	if not json.has("width") or not test_float(json["width"]):
		return false
	if not json.has("speedlimit") or not test_float(json["speedlimit"]):
		return false
	...
	if json.has("links"):
		...
		for link in json["links"]:
			...
			if not link.has("lane") or not test_string(str(link["lane"])):
				return false
			if not link.has("hasPrio") or not test_bool(link["hasPrio"]):
				return false
			if not link.has("isOpen") or not test_bool(link["isOpen"]):
				return false
			if not link.has("hasFoe") or not test_bool(link["hasFoe"]):
				return false
			if not link.has("state") or not test_string(str(link["state"])):
				return false
...
func road(json) -> bool:
	...
	if not json.has("streetName") or not test_string(str(json["streetName"])):
		append_error("Road " + str(json["id"]) + " has no or invalid streetName")
		return false
...
func vehicle_addition(json) -> bool:
	...
	# Optional: slope
	if json.has("slope") and not test_float(json["slope"]):
		append_error("VehicleAddition with id " + str(json["id"]) + " has invalid slope")
		return false
